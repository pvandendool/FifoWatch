...
namespace S7CommPlusGUIBrowser
{
    public partial class Form1 : Form
    {
        ...
        private PlcTag m_CurrentTag = null;
        ...
        private void readTagBySymbol()
        {
            ...
            // Pre-fill write box with the raw value (strip the "XX: " quality prefix)
            m_CurrentTag = tag;
            string tagStr = tag.ToString();
            int sepIdx = tagStr.IndexOf(": ");
            tbWriteValue.Text = sepIdx >= 0 ? tagStr.Substring(sepIdx + 2) : tagStr;
        }

        private void writeTagBySymbol()
        {
            if (conn == null || tbSymbol.Text == "") return;

            setStatus("writing...");
            PlcTag tag = conn.getPlcTagBySymbol(tbSymbol.Text);
            if (tag == null)
            {
                setStatus("error: tag not found");
                return;
            }

            string input = tbWriteValue.Text;
            try
            {
                if (tag is PlcTagBool t)        t.Value = input == "1" || string.Equals(input, "true", StringComparison.OrdinalIgnoreCase);
                else if (tag is PlcTagByte tb)  tb.Value = byte.Parse(input);
                else if (tag is PlcTagUSInt tu) tu.Value = byte.Parse(input);
                else if (tag is PlcTagSInt ts)  ts.Value = sbyte.Parse(input);
                else if (tag is PlcTagChar tc)  tc.Value = input.Length > 0 ? input[0] : '\0';
                else if (tag is PlcTagWChar twc) twc.Value = input.Length > 0 ? input[0] : '\0';
                else if (tag is PlcTagWord tw)  tw.Value = ushort.Parse(input);
                else if (tag is PlcTagUInt tui) tui.Value = ushort.Parse(input);
                else if (tag is PlcTagInt ti)   ti.Value = short.Parse(input);
                else if (tag is PlcTagDWord tdw) tdw.Value = uint.Parse(input);
                else if (tag is PlcTagUDInt tud) tud.Value = uint.Parse(input);
                else if (tag is PlcTagDInt tdi) tdi.Value = int.Parse(input);
                else if (tag is PlcTagReal tr)  tr.Value = float.Parse(input);
                else if (tag is PlcTagLReal tlr) tlr.Value = double.Parse(input);
                else if (tag is PlcTagLInt tli) tli.Value = long.Parse(input);
                else if (tag is PlcTagULInt tul) tul.Value = ulong.Parse(input);
                else if (tag is PlcTagLWord tlw) tlw.Value = ulong.Parse(input);
                else if (tag is PlcTagString tstr)   tstr.Value = input;
                else if (tag is PlcTagWString twstr) twstr.Value = input;
                else
                {
                    setStatus("error: write not supported for this data type");
                    return;
                }
            }
            catch (FormatException)
            {
                setStatus("error: invalid value format");
                return;
            }
            catch (OverflowException)
            {
                setStatus("error: value out of range");
                return;
            }

            int res = conn.WriteTags(new[] { tag });
            if (res != 0)
            {
                setStatus("error: " + S7Client.ErrorText(res));
                return;
            ...
            setStatus("write OK");
            readTagBySymbol(); // refresh read value after write
        }

...
namespace S7CommPlusDriver
{
    public partial class S7CommPlusConnection
    {
        ...
        public int Browse(out List<VarInfo> varInfoList)
        {
            ...
            var containerObjs = objs.GetObjects();
            vars.SetTypeInfoContainerObjects(containerObjs);
            // Populate the per-relid cache so getTypeInfoByRelId can find types
            // that can't be fetched individually (e.g. some optimized DBs).
            foreach (var co in containerObjs)
                if (typeInfoList.Find(t => t.RelationId == co.RelationId) == null)
                    typeInfoList.Add(co);
            vars.BuildTree();
            vars.BuildFlatList();
